...
## Format of input
- A nonlinear equation, where the viable functions are log (natural), exp, sin, cos.
- There has to be a star (*) operator in between coefficients and variables.
- The hat (^) operator is used only for constant exponents, exp(.) for exponents containing variables.
